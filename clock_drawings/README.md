...
Raw .tif scans
      │
      ▼
image_preprocess.ipynb   ← Binarize, clean artifacts, comparison output
      │
      ▼
clock_processor.ipynb    ← Clean + island-crop to isolate clock drawing
      │
      ▼
create_dataset.py        ← 80/10/10 train/test/valid split
      │
      ▼
cnn_analysis_0to1.ipynb  ← Binary classification (affected vs. not)
cnn_analysis_0to5.ipynb  ← 6-class classification (scores 0–5)
```
...
| File | Description |
|------|-------------|
| `image_preprocess.ipynb` | Initial preprocessing: converts to B&W, removes dirt/rectangles/edge artifacts, saves side-by-side comparison TIFs |
| `clock_processor.ipynb` | Full preprocessing: same cleaning as above + island-crop to isolate the clock, saves both comparison and cropped output |
| `create_dataset.py` | Splits cropped TIFs into train/test/valid folders (80/10/10) |
| `cnn_analysis_0to1.ipynb` | Binary CNN: classifies as *affected* (scores 0–2) vs. *not affected* (scores 3–5) |
| `cnn_analysis_0to5.ipynb` | Multiclass CNN: classifies into 6 score categories (0–5) |
